...
### Part 1: Create "Fake" data based on a function y = x^3 + epsilon
...
# I am going to randomly assign a number to the data
fake_data$random_unif <- runif(nrow(fake_data))
learning_sample       <- subset(fake_data, random_unif<=0.3) 
test_sample           <- subset(fake_data, random_unif>0.7)

# Other ways to get the train and test data
# train           <- sample(1:nrow(fake_data), 500)
# learning_sample <-  fake_data[-train,]
# test_sample     <- fake_data[train,]


# Plot the y on x for both the learning and test sample

plot(y ~ x, data = learning_sample, las = 1)
plot(y ~ x, data = test_sample, las = 1)
...
######
# F. The code below estimates a model using k nearest neighbour, and predicts y
######
...
######
# G. Recalculate the mean square error for the prediction with k nearest neighbour
######

MSE_kkn_test <-  mean((test_sample$y-kkn_predict_y)^2)
